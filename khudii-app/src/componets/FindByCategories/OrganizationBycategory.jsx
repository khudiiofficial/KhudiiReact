  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setOrganizations(res.data.reverse()); // replace with res.data later
      } catch (error) {
        console.log(error);
      ...
      <div className="max-w-[1240px] mx-auto">
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {organizations.map((org) => (
            <div onClick={()=>{nav(`/organization/${org.slug}`,{state:{id:org.id}})}} key={org.id} className="org-card">
...
></div>
                ...
                <button  className="inline-flex rounded-[25px] w-auto px-5 py-2.5 text-sm font-medium bg-[#E3001C] text-white transition-all duration-300 focus:outline-none">
